...
export class ShippingResolver {
  ...
  @UseGuards(AuthGuard)
  get(@Args('searchInput') searchInput: SearchInput) {
  ...
  @UseGuards(AuthGuard)
  getSingle(@Args('id', { type: () => String }) id: string) {
  ...
  add(@Args('shippingInput') shippingInput: ShippingInput) {
  ...
  @Roles(Role.ADMIN)
  @UseGuards(AuthGuard, RolesGuard)
  status(@Args('id', { type: () => String }) id: string) {
