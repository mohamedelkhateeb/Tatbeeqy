...
export class ReviewResolver {
  ...
  getReviews(@Args('productId', { type: () => String }) productId: string) {
  ...
  getReviewsByAdmin(@Args('searchInput') searchInput: SearchInput) {
  ...
  @Roles(Role.SELLER)
  @UseGuards(AuthGuard, RolesGuard)
  reply(@Args('replyInput') replyInput: ReplyInput) {
  ...
  @Roles(Role.EDITOR, Role.MODERATOR, Role.ADMIN)
  @UseGuards(AuthGuard)
  visibility(@Args('reviewId', { type: () => String }) reviewId: string) {
