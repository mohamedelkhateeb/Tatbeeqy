...
export class ProductService {
  ...
  async getFlashProduct(flashId: string, searchInput: SearchInput) {
    ...
    if (searchInput.search) {
      products.where('LOWER(product.name) LIKE :search', {
    ...
    if (productInput.discountUnit === 'percent') {
      totalPrice = Math.round(
        Number(productInput.price) -
          Number(productInput.price) * (Number(productInput.discount) / 100),
      )
    } else if (productInput.discountUnit === 'flat') {
      totalPrice = Math.round(
        Number(productInput.price) - Number(productInput.discount),
      )
    }
    let attributes = null
  ...
  async updateFlash(
    updateFlashProductInput: UpdateFlashProductInput,
    reqUser: ReqUser,
  ) {
      ...
      if (flash.discountUnit === 'percent') {
        totalPrice = Math.round(
          Number(product.price) -
            Number(product.price) * (Number(flash.discount) / 100),
        )
