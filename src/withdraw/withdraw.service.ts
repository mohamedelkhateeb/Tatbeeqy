...
export class WithdrawService {
  ...
  constructor(
    ...
    @InjectRepository(Seller) private sellerRepository: Repository<Seller>, // @InjectRepository(Platform) private platformRepository: Repository<Platform>
  ) {}
  ...
  async getByAdmin(sellerId: string, searchInput: SearchInput) {
    ...
    if (searchInput.search) {
      withdraws.where('LOWER(withdraw.name) LIKE :search', {
  ...
  async getIncome(searchInput: SearchInput, reqUser: ReqUser) {
    ...
    if (searchInput.search) {
      incomes.where('LOWER(income.name) LIKE :search', {
        search: `%${searchInput.search.toLowerCase()}%`,
      })
    }
  ...
  async release(paymentInput: ReleasePaymentInput, reqUser: ReqUser) {
    ...
    total = Math.round(total - total * (Number(100) / 100))
  ...
  async confirm(withdrawId: string) {
    ...
    if (result.affected === 0)
      throw new NotFoundException('Withdraw release not found!')
    return {
      success: true,
