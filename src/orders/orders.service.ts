...
export class OrderService {
  ...
  constructor(
    ...
    private productRepository: Repository<OrderProduct>,
    @InjectRepository(OrderSeller)
    private orderSellerRepository: Repository<OrderSeller>,
    ...
    @InjectRepository(Refundable)
    private refundableRepository: Repository<Refundable>,
  ...
  async getOrders(searchInput: SearchInput) {
    ...
    if (searchInput.search) {
      orders.where('LOWER(order.orderId) LIKE :search', {
  ...
  async getOrderBySeller(reqUser: ReqUser, searchInput: SearchInput) {
    ...
    if (searchInput.search) {
      orderSellers.andWhere('LOWER(orderSeller.shopName) LIKE :search', {
        search: `%${searchInput.search.toLowerCase()}%`,
      })
    }
  ...
  async adminStatus(orderSellerId: string, status: string) {
    ...
    if (orderSeller.status === 'Delivered')
      throw new NotFoundException("You can't change a delivered order status!")
    if (orderSeller.status === 'Cancelled')
      throw new NotFoundException("You can't change a cancelled order status!")
    const order = await this.orderRepository.findOne({
      where: {
    ...
    if (status === 'Delivered') {
      ...
      if (userPoint) {
        await this.userPointRepository.increment(
          ...
          'points',
          Math.round(orderSeller.price / 10),
        )
      } else {
